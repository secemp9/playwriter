...
Each session is an **isolated sandbox** with its own `state` object. State is isolated per session, and browser tabs are isolated per git worktree, so sessions in different worktrees never see each other's tabs.
